# Tugas Kelompok 5: Hardening Keamanan Transmisi Data REST API Berbasis Docker Container Menggunakan TLS 1.3 Berpola Cipher Suite AES-128-GCM Terhadap Serangan Adversary-in-the-Middle
...
2. Implementasi Defense in Depth: Mengamankan sistem dengan perlindungan ganda (*Layered Security*):
   - Data in Transit: Mengamankan lalu lintas jaringan dari *sniffing* menggunakan Nginx Reverse Proxy sebagai terminasi TLS 1.3 dengan standar *cipher suite* kriptografi `TLS_AES_128_GCM_SHA256`.
   - Data at Rest: Menerapkan *Application-Level Encryption* menggunakan algoritma AES-128-GCM menggunakan library standar *Python Cryptography*. Data sensitif (seperti Gaji Karyawan pada modul CRUD) dienkripsi sebelum masuk ke disk/database, melindungi data meskipun *database server* (MySQL) berhasil dibobol (*post-breach*).
3. Analisis Berbasis Standar Industri: Mengevaluasi kelemahan dan validasi mitigasi berdasarkan pemetaan standar MITRE ATT&CK (Teknik T1557.001) dan risiko global OWASP Top 10:2025 (A02:2025 Cryptographic Failures) secara terukur menggunakan aplikasi *packet analyzer* (Wireshark).
...
Sertifikat diperlukan agar Nginx Proxy dapat menjalankan mode HTTPS (TLS 1.3). Jalankan perintah berikut jika belum terdapat file `server.key` dan `server.crt` di direktori `demo/nginx/certs/`:
...
3. Simulasi Transmisi Data:
   - Buka *browser* di mesin Host lalu kunjungi WebTop Client: `http://localhost:3000`
...
4. Analisis di Wireshark (Untuk membuktikan kerentanan):
...
4. Analisis di Wireshark (Untuk membuktikan mitigasi):
...
- Buktikan bahwa nilai kolom `salary_encrypted` tidak menampilkan nominal numerik, melainkan string acak *Base64-encoded ciphertext*. Fitur Enkripsi tingkat aplikasi (*Application-Level Encryption* AES-128-GCM) bekerja secara sempurna untuk menyandikan data sensitif di level Backend.
